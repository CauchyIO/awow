...
# /design-review — drive or capture a solution-design session
...
- **Presentation** → `proposals/<topic>/` containing `questionnaire.md → background.md → slides.md → slides.html`. Question-first; never jump to slides.
...
For any blocker discovered in 2.4 that crosses into a neighbouring team's domain, propose an `ESCALATE` action per `/process-transcript` Phase 2.4 conventions. Do not silently absorb cross-team work into a child item.
...
`/design-review` produces a design artefact. If the artefact is a proposal with a decomposed work-item tree, the downstream `/proposal-from-board` (or equivalent) turns it into board items at scale. The shape produced in Phase 3.2 is designed to be consumable by that downstream skill without manual reshaping.
