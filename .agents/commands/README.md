...
| `spread` | Opt-in via `/awow-add <command>` (after first Seed cycle) | `board-skill`, `coaching-review`, `solution-design-flow`, `project-plan`, `design-system`, `artifact`, `my-work` |
| `standardise` | Opt-in via `/awow-add <command>` (most of team active) | `daily-checkin`, `daily-digest`, `weekly-digest`, `cross-team-view`, `project-manager` |
