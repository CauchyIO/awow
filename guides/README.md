...
| Guide | What it covers |
|---|---|
| [The core delivery loop](guide-core-delivery-loop.md) | Two Seed commands from board to PR, the `/daily-checkin` cap, and the look-first, propose-then-approve spine through all of them. |
...
| [Transcript routing](guide-transcript-router.md) | One entry point reads the transcript, recommends a specialist, and gates before anything reaches the board. |
...
| [Design system & artifacts](guide-design-system-and-artifacts.md) | Stand a house style up once, then render every deck and one-pager from it. Opt-in. |
| [Session correlation](guide-session-correlation.md) | Linking agent-authored board entries back to their session trace via a footer id. |

## Pillar · cross-team — center of gravity: the service RACI

| Guide | What it covers |
|---|---|
| [Cross-team & pillar](guide-cross-team-and-pillar.md) | Why a pillar is a different archetype, not a bigger team: three mechanisms across the seam, spined on one activity×team RACI. |
| [Program portfolio view](program-portfolio-view.md) | Allocation under constraint — priority routes down to the team boards, size and progress roll back up. |
