...
Follow `{HUB}/context/tooling/activity-collection.md`, falling back to `{AWOW_ROOT}/context/tooling/activity-collection.md` (a vendored copy wins over the shipped one): **reuse `activity/YYYY-MM-DD.json` if it already exists for the day, otherwise produce it.** That step owns the board / code / chat queries (all keyed off `{HUB}/context/tooling/board.md`), the normalised snapshot schema, and the private-team gate — so you do not re-query per lens, and the private-team exclusion is already applied. Scope every board query to the board-team filter in `{HUB}/context/tooling/board.md` when one is declared; the digest covers this installation's slice only.
