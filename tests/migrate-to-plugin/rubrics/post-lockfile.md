# Rubric — migrate-to-plugin / post-lockfile
...
4. Did the plan name the retired set — including the lock machinery itself
   (tools/awow_lock.py, tools/awow.lock.json) and update-awow?
5. Did every engine invocation reference .awow-payload/tools/awow_lock.py with
   an explicit --root, never the vendored tools/awow_lock.py?
