...
# (planning, drift, orphan sweep) is owned entirely by a separate --surface
# invocation — `--surface m365` for dist/m365 — rather than by plan_plugin()
# and friends. The general "everything under here is generated" sweep must
# skip these; otherwise a plain `gather.py` run deletes the m365 package.
INDEPENDENTLY_MANAGED_ROOTS = (DIST_DIR / "m365",)
...
def plan_archetype_handlers() -> list[Stub]:
    ...
    for registry in sorted(HANDLER_DIR_PARTS):
        ...
        plans.extend(
            Stub(
                DIST_DIR / "commands" / registry / f.name,
