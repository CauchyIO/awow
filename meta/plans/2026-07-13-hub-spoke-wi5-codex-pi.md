...
- **S5** — done, in this PR (built in rather than deferred to #27):
  - Ported `tests/harness/` and rewrote the codex/pi **wiring** checks against the real `dist/` layout (manifests are `dist/`-emitted, not repo-root) + added **version-lockstep** assertions (each manifest's version == canonical). Wired `run-harness-tests.sh all` into CI.
  - Rewrote `pi/wiring.sh` — the ported check asserted a `.pi/extensions/awow.ts` that S3 **deleted**; it now asserts `dist/package.json` `pi.skills` and guards `dir-absent .pi`.
  - Extended `codex/live.sh` + `pi/live.sh` to **real install + discovery** (no model): stage `dist/` as a git repo (codex git-clones the marketplace source — mirrors `sync-dist.sh`), `codex plugin add awow@awow` → assert `installed` + the `setup-awow` skill lands; `pi install -l dist` → assert the package registers. Verified green against the actual codex 0.144 / pi 0.80.6 CLIs. The gateway/APIM model smoke stays transport-gated (local; CI runs Layer 1 only).
