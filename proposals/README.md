...
| [repo-canonical-verdict](repo-canonical-verdict.md) | **Draft** (2026-09-16) | Project-canonical capture: an anchored repo declares its OKF bundle root (`knowledge:` in the connector), registration adopts existing docs, and the reference-before-capture seam gains a fourth verdict so repo facts land in the repo, not the anchor. Decisions taken 2026-09-16: verdict only, awow owns no write path into the repo; per-repo inbox/drain declined, not parked. CAU-1612. |
