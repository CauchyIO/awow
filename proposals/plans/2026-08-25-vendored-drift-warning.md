...
> **Executed 2026-08-30 with the spec's §9 amendments.** Task 0's CAU-1335 precondition was
> dropped (A-1: the doc landed in `.agents/AGENTS.md`, `test_wrong_root_guard.py` left out of
> the battery); Task 2's maintainer marker requires `"name": "awow"` (A-2) and the
> `build_context` branch keys on "not a spoke" rather than `adopted` (A-3); Task 3 targets
> AGENTS.md instead of the contract file. The literal code below is the pre-amendment draft —
> the committed source is authoritative.
