...
**Several `board.md` candidates, or an index-form `board.md`?** Resolve per the context-resolution contract (`{HUB}/context/tooling/context-resolution.md`, falling back to `${CLAUDE_PLUGIN_ROOT}/context/tooling/context-resolution.md`) before any board read or write: the nearest installation inside the repo boundary — never a parent or sibling repo's — then, for an index, the ladder (explicit reference → scope match → session pin → spoke board scope → invoker default → picker), recording the answer in `.awow/board-session.md` beside the absent-board note. Announce a silently-resolved target with `targeting board: <name>` before the first write, and honour the contract's write boundary: a write crossing a git repo boundary needs the user's answer to a question naming both repos — a generic plan approval is not that answer.
