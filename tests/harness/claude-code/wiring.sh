...
wiring() {
  ...
  # No generated stubs in the in-repo harness folders (AWO-257): commands and
  # skills reach Claude Code through the plugin payload, so a stub reappearing
  # here would list every command twice in the picker.
  file-absent "$r/.claude/commands/setup-awow.md"
  dir-absent "$r/.claude/skills"
  file-exists "$r/.claude/CLAUDE.md"
  file-not-contains "$r/.claude/CLAUDE.md" 'GENERATED by tools/gather.py'
  file-contains "$r/.claude/CLAUDE.md" '\.agents/AGENTS\.md'
  # The one hand-authored repo-local command: the maintainer eval runner.
  file-exists "$r/.claude/commands/test-awow.md"

  # dist/ must be in sync with .agents (drift guard)
  if ( cd "$r" && python3 tools/gather.py --surface plugin --check >/dev/null 2>&1 ); then
    _record pass "gather --surface plugin --check"
  else
    _record fail "gather --surface plugin --check"
  fi
