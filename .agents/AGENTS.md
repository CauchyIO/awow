...
`{{HUB}}` and `{{PROJECT}}` resolve to "the repo root" only when CWD sits inside exactly one scaffolded repo. Everything beyond that trivial case — sibling repos under one working root, a nested checkout, a monorepo with several context trees, an index-form `board.md`, the invoker profile, the spoke board scope, and the write boundary for awow artefacts — is governed by the **context-resolution contract**: read `{HUB}/context/tooling/context-resolution.md` first, falling back to `{AWOW_ROOT}/context/tooling/context-resolution.md` (in this repo both are `context/tooling/context-resolution.md`). Follow it before any board read or write whenever resolution is not trivially unambiguous, and before any awow artefact write that would leave the current repo.
