...
# --- the mapped anchor (post-rename spelling) is an allowed root ------------
anchor_clone = _repo(ws, "anchor-clone")
with open(os.path.join(repo_a, ".awow", "anchor.json"), "w") as f:
    json.dump({"remote": "git@example.com:org/anchor.git", "path": anchor_clone}, f)
out, _ = _run(os.path.join(anchor_clone, "context", "knowledge-base", "d.md"), cwd=repo_a)
check("anchor write allowed via .awow/anchor.json", _decision(out) is None, out)
out, _ = _run(os.path.join(hub, "context", "knowledge-base", "d.md"), cwd=repo_a)
check("anchor.json wins over the legacy hub.json beside it", _decision(out) == "ask", out)
os.remove(os.path.join(repo_a, ".awow", "anchor.json"))
anchor2 = _repo(ws, "anchor-two")
out, _ = _run(os.path.join(anchor2, "context", "knowledge-base", "d.md"), cwd=repo_a,
              extra_env={"AWOW_ANCHOR": anchor2})
check("anchor write allowed via $AWOW_ANCHOR", _decision(out) is None, out)
