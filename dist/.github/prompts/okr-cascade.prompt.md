...
layer: department
prerequisites:
  - "context/department/ stood up and at least one team joined (`/setup-department`)"
  - "an OKR doc for the current quarter at `context/department/okrs-<quarter>.md`"
...
1. Ask which PI is being planned (e.g. `2026-PI-4`), mirroring `/setup-department`'s quarter ask. If `context/department/pi-plan-<PI>.md` does not exist for that PI, copy `context/department/templates/pi-plan.md` to that path and update its heading to name the actual PI; if it exists, read it.
