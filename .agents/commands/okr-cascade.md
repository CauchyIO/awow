...
2. **Run the check.** Run `python {AWOW_TOOLS}/cascade_check.py --json` from the repo root. Parse the JSON body: `quarter`, `findings` (list of `{class, team, detail}`), `drift` (list of `{team, pinned, remote}`), `pin_age_days` (map of team → integer days).
