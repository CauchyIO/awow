...
You own the mechanics only. `/artifact` decides content, reads the design system and works the board; you turn the agreed markdown into each target and prove it before anyone calls it done. Read `{ANCHOR}/context/tooling/design-system.md` (falling back to `../../context/tooling/design-system.md`) for `templates_dir` and `word_reference`; never invent styling.
