...
wiring() {
  ...
  file-contains "$r/dist/opencode/awow/agent-skills/setup-awow/SKILL.md" 'Connect a repo'
...
raise SystemExit(0 if m == './opencode/awow/.opencode/plugins/awow.js' else 1)"
...
raise SystemExit(0 if json.load(open('$p')).get('type') == 'module' else 1)"
  ...
  cmd-succeeds "pi.skills still registers ./pi/awow/agent-skills" -- python3 -c "
import json
...
raise SystemExit(0 if './pi/awow/agent-skills' in skills else 1)"
  ...
  file-exists "$r/dist/opencode/awow/agent-skills/using-awow/SKILL.md"
