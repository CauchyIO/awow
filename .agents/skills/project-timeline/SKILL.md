...
The engine is `{AWOW_TOOLS}/session_timeline.py` (with `session_timeline_template.html` for the view and `{AWOW_TOOLS}/mlflow_reader.py` — the shared, schema-validated reader also used by `awow-usage-coach`). This skill is the judgment: scope, cost-gating, reading the picture, and coaching — the script never decides scope on its own.
...
3. **Build the timeline.** Run the tool and report the session count (and, for MLflow, the projects it split into):
   ...
   python {AWOW_TOOLS}/session_timeline.py --project-path <repo> --out <out-dir> --tz-offset <hours>
   # MLflow export (→ index.html + one timeline-<slug>.html per project; optionally pre-scope a user):
   python {AWOW_TOOLS}/session_timeline.py --mlflow-export <mlflow_export-dir> --out <out-dir> --tz-offset <hours> [--user <name>]
   ```
