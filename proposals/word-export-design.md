...
**Status:** Implemented 2026-09-04 — [PR #87](https://github.com/CauchyIO/awow/pull/87), board item CAU-1525. Build amendments in §10, review amendments in §11; the AC3 manual walk is done (§11.15).
...
15. **AC3 verified by manual walk, 2026-09-04.** Cauchy had no Word template (only a PowerPoint one), so one was built from the v2 design tokens — Nunito body and Nunito SemiBold headings (weight cap 600, no bold flag), text ramp `1A1A1A`/`6B6B6B`/`9A9A9A`, link `337FF9`, light table borders `E0DFDD`, A4 with 2.5 cm margins, and a footer carrying the wordmark in the reserved accent `F24B37` plus the page number. Walk against this branch's §3.2b: `pandoc --version` exit 0; the probe over a three-line sample exits 0 and the output carries the footer part, the Nunito faces and the page setup; a `.dotx` is refused by extension with the save-as instruction and nothing is converted; a non-docx handed as the template makes the probe exit non-zero and `word_reference` stays empty with pandoc's message reported. Rendering this spec and the eval brief through the reference doc gives the same outlines as the stock-style runs. The reference doc was built only to exercise AC3 and is not landed anywhere: Cauchy itself works in markdown and HTML, so the Word target exists for adopters.
