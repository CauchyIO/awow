...
**Status:** Implemented 2026-08-30 with three amendments (§9). Draft by Arie, 2026-08-25. Derived from [CAU-1338.md](CAU-1338.md), which carries the
...
## 9. Amendments at implementation (2026-08-30)

Three departures from the sections above, each reviewed and approved before the build; the
sections above are otherwise implemented verbatim.

- **A-1 Decoupled from CAU-1335 (PR #77).** The dependency was the doc section's *home*
  (`context/tooling/context-resolution.md`), which does not exist on main; #77 was open and
  stale at implementation time, and blocking a mechanical fix on a 45-file PR is the wrong
  dependency direction. §4.1's substance landed as the **Machinery vintage** paragraph in
  `.agents/AGENTS.md` §Reading machinery, directly after the precedence rule it qualifies; §4.2's
  pointer sentence is deferred. When #77 lands, the paragraph moves into the shipped contract and
  the pointer replaces it — the same pattern #77 uses. `tests/hooks/test_wrong_root_guard.py` is
  dropped from the close-out battery for the same reason. Consequence for AC2: until then the
  rule reaches adopters through the hook message itself (which states the precedence and the
  remedy), not through a shipped context file.
- **A-2 The maintainer marker is the *awow* plugin manifest, not any plugin manifest.** §3.2
  D2–D4 keyed on bare presence of `.claude-plugin/plugin.json`; a plugin repo that vendored
  awow pre-CAU-1340 would have been told `unstamped (predates build stamps)` every session with
  a remedy that does not apply. `maintainer_checkout()` parses the manifest and requires
  `"name": "awow"`; anything else falls through to the adopter branch. One new hook check
  covers it (a non-awow plugin repo that vendored awow stays silent).
- **A-3 The tier is not gated on `adopted`.** §3.1 hung the drift branch off `adopted`
  (`.agents/AGENTS.md` present). AWO-258 re-keys `adopted` on the installation root; hanging
  the tier on it would let that change silently enable or disable drift detection. The tier now
  runs for every non-spoke repo and the decision table's own markers decide — D6 silence is the
  default, so plugin-first adopters and plain repos see no change. The setup-nudge `elif` is
  byte-identical to before.

Recorded, not acted on: the maintainer warning's bidirectionality (D-4) will render in most
feature-branch sessions of this repo that touched `dist/` until the plugin cache syncs. That is
the intended AWO-156 signal; if it proves too loud, order by the version field when versions
differ and reserve "differs" for equal versions.
