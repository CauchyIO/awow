...
phase: standardise
prerequisites:
  - "context/kb-inbox/ holds one or more candidate files (produced by /kb-mine or /daily-routine)"
  - "context/knowledge-base/ set up (the durable layer the drain writes into)"
...
You run the **synthesis drain**: read the committed candidates in `context/kb-inbox/`,
decide each one's fate against the existing KB, and — **on explicit approval** — promote
the survivors into `context/knowledge-base/`. This is the back half of the pipeline whose
...
[`context/knowledge-base/synthesis.md`](../../context/knowledge-base/synthesis.md). This
...
Phase 2 ─ GATE (approval)  ──→ promote / annotate / no-op / drop  ──→ context/knowledge-base/
```
...
Read the **committed** candidate files in `context/kb-inbox/` (skip `README.md` and
...
[`context/knowledge-base/synthesis.md`](../../context/knowledge-base/synthesis.md)
...
2. Leave a `Promoted to context/knowledge-base/<subfolder>/<x>.md` pointer in the source
...
- **Gated writes only.** No write into `context/knowledge-base/`, no board comment, and no
