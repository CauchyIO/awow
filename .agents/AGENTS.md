...
`{{HUB}}` is the pre-rename spelling of `{{ANCHOR}}`: the build and the machinery still honour it in adopter-owned files, new writes always use `{{ANCHOR}}`.
...
**Machinery vintage.** The `{{ANCHOR}}`-first rule keys on file presence, so a vendored install runs whatever vintage the current checkout carries — a branch cut before an upstream change, or an install never updated, wins exactly like a deliberate edit, silently serving the older content. Every built payload names its vintage in `.claude-plugin/build.json` (the canonical version plus a digest of the payload content, displayed `<version>+<digest>`, so two rebuilds of one version are distinguishable), and the session-start hook compares the installed payload's stamp against the checkout — this repo's `dist/` stamp, or a legacy vendored repo's `tools/awow.lock.json` vintage — and surfaces a one-line `awow drift:` warning naming both sides when they differ. `/migrate-to-plugin` retires a legacy vendored tree; in this repo, `python tools/gather.py && claude --plugin-dir dist` runs the checkout's own payload.

**Fill on first need.** An absent team-data file is never a stop and never a hard prerequisite. The command that needs it makes one plain-language offer at that moment — fill it now (drafted from the repo and board where possible), proceed on the shipped default, or skip and note it — and continues with the default on silence. A "fill it now" yes is the approval for that context write. `/setup-awow` remains the place to wire what cannot be defaulted (installer, board surface, anchored-repo registration) and to deepen any fill later.

In an anchored repo — one whose root `AGENTS.md` frontmatter names its anchor by remote URL (`anchor:`; `hub:` pre-rename) — resolve `{{ANCHOR}}` through `$AWOW_ANCHOR` (else the pre-rename `$AWOW_HUB`), else the gitignored `.awow/anchor.json` link written at registration (`.awow/hub.json` pre-rename), after verifying the recorded clone's `origin` still matches the connector's remote. A missing or out-of-sync link is a prompt to the user to (re)map it in an interactive session and a loud stop in a headless one — never scan for candidates, never guess a location, never improvise conventions.

## Context resolution — which installation, which board

`{{ANCHOR}}` and `{{PROJECT}}` resolve to "the repo root" only when CWD sits inside exactly one scaffolded repo. Everything beyond that trivial case — sibling repos under one working root, a nested checkout, a monorepo with several context trees, an index-form `board.md`, the invoker profile, the anchored board scope, and the write boundary for awow artefacts — is governed by the **context-resolution contract**: read `{ANCHOR}/context/tooling/context-resolution.md` first, falling back to `{AWOW_ROOT}/context/tooling/context-resolution.md` (in this repo both are `context/tooling/context-resolution.md`). Follow it before any board read or write whenever resolution is not trivially unambiguous, and before any awow artefact write that would leave the current repo.

The contract is machinery and ships in every payload — that is the point. Do not restate its rules here: this section stays a pointer so the shipped contract and these maintainer instructions cannot drift (CAU-1335: the ladder used to live only in this file, which no payload ships, so no adopter session could read it).
