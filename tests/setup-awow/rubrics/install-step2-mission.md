...
Step 2 asks for the team's mission in one sentence. Script supplies the sample team's mission line, then `approve` to land it.
...
4. Did the wizard accept the sample mission sentence on the first try (it names audience, change, and constraint, so the "refuse anything trivial" rule should not fire)?
